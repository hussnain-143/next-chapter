...
exports.hookLocalDbFallback = void 0;
...
const decorateInstance = (instance, tableName) => {
    if (!instance)
        return instance;
...
const hookLocalDbFallback = () => {
    ...
    modelNames.forEach((modelName) => {
        try {
                ...
                if (query && typeof query === 'object') {
                    Object.keys(query).forEach((key) => {
                        ...
                        if (val && typeof val === 'object' && ('$in' in val || '$gte' in val || '$lt' in val)) {
                            if ('$in' in val) {
                                const arr = val['$in'];
                                list = list.filter((item) => arr.includes(String(item[key])));
                            }
                            // Simple date range match logic for weekly heatmap aggregate
                            if ('$gte' in val) {
                                const dateLimit = new Date(val['$gte']).getTime();
                                list = list.filter((item) => new Date(item[key]).getTime() >= dateLimit);
                            }
                            if ('$lt' in val) {
            ...
            Model.findOne = function (query = {}) {
                ...
                const promise = queryResult.then((list) => list[0] || null);
            ...
            Model.deleteMany = async function (query = {}) {
                ...
                if (Object.keys(query).length === 0) {
                    ...
                    if (val && typeof val === 'object' && '$in' in val) {
                        ...
                        db[tableName] = table.filter((item) => !arr.includes(String(item[field])));
                    }
                    else {
                        db[tableName] = table.filter((item) => String(item[field]) !== String(val));
                    }
                }
                saveDb();
    ...
    const saveOrig = mongoose_1.default.Model.prototype.save;
    mongoose_1.default.Model.prototype.save = async function (options) {
...
exports.hookLocalDbFallback = hookLocalDbFallback;
